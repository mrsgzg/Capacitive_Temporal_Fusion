# -*- coding: utf-8 -*-
"""
LSTM Baseline for Liquid Classification
Simple LSTM with attention mechanism for sequence classification
"""
...
class LSTMBaseline(nn.Module):
    """Simple LSTM with attention for liquid classification"""
    
    def __init__(self, seq_len, in_ch, hidden_size, num_layers, dropout, num_classes):
        super().__init__()
...
def train_lstm(train_loader, val_loader, num_classes, device, args):
    """Train LSTM model"""
    ...
    for epoch in range(args.epochs):
        ...
        # Validate
        val_acc, val_f1, _, _ = evaluate_lstm(model, val_loader, device)
...
def main():
    ...
    if args.out_dir is None:
        ...
        args.out_dir = f"baseline_lstm_{timestamp}"
    ...
    # Train
    print("[INFO] Training...")
    model, best_acc, history = train_lstm(train_loader, val_loader, len(all_classes), device, args)
    
    # Final evaluation on val set
    print("[INFO] Final evaluation...")
    model.load_state_dict(torch.load(os.path.join(args.out_dir, "best_model.pt"))["model"])
    val_acc, val_f1, preds, labels = evaluate_lstm(model, val_loader, device)
    ...
    print("\n" + "="*80)
    print("LSTM BASELINE - LIQUID CLASSIFICATION")
    print("="*80)
    ...
    with open(os.path.join(args.out_dir, "results.txt"), "w") as f:
        f.write("="*80 + "\n")
        f.write("LSTM BASELINE - LIQUID CLASSIFICATION\n")
        f.write("="*80 + "\n")
